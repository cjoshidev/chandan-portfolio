---
title: "The UK paid £4.1 million for a bookmarks site"
date: "2023-10-27"
description: "The UK Government recently unveiled its 'AI Skills Hub', which wants to provide 10 million workers with AI skills by 2030."
tags: ["industry", "opinion"]
---

The UK Government recently unveiled its 'AI Skills Hub', which wants to provide 10 million workers with AI skills by 2030. The main site was delivered by PwC for the low, low price of.. **£4.1 million (~$5,657,000)**.

## The Cost

### II.1.7) Total value of the procurement (excluding VAT)
Value excluding VAT: **£4,148,645**

It is *not* good. Like, at all - the UI is insanely bad and it's clear that this was just a vibecoded site (to be fair, this is the *AI* Skills Hub, but c'mon, where is the pride in your work? I would be ashamed to even release this as a prototype!)

There is literally no way to justify that price tag. It's essentially a collection of links to other resources. A bookmarks site.

> "A billion here, a billion there, and pretty soon you're talking about real money."

## Technical Analysis

The site appears to be built using... actually, does it matter? The fundamental issue isn't the stack, it's the value for money.

1.  **Complexity**: Low. It's a directory.
2.  **Design**: Standard government/consultancy chic.
3.  **Performance**: Acceptable, but nothing to write home about.

*Note: This is a sample post replicated from the inspiration site for testing purposes.*
